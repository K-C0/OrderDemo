...
public class OrderServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		}else if(method != null && method.equals("pay")) {   //  点击“付款”
...
//			response.sendRedirect(getServletContext().getContextPath()+"/app/index.do");
			...
			response.sendRedirect(getServletContext().getContextPath()+"/app/index.do");
		}else if(method != null && method.equals("list")) {   //订桌时  method = list
			HttpSession  session = request.getSession();
			User user = (User) session.getAttribute(ConstantUtil.SESSION_NAME);
			//查询所有的订单
			List<Order>  orders = orderService.findAll(user.getId());//查询userid的所有订单
